<template>
  <div class="selectionBar" @click="toggleSelection" ref="selectionBarRef">
    <div style="display: flex">
      ...
      <img
        :src="
          isOpen ? '/src/assets/arrowBottom.svg' : '/src/assets/arrowBottom.svg'
        "
    ...
    <div v-if="isOpen" style="width: 100%; margin-top: 10px">
      ...
      <div class="variantText" @click="selectOption('Один из списка')">
        Один из списка
      </div>
      <div class="variantText" @click="selectOption('Несколько из списка')">
        Несколько из списка
      </div>
    </div>
  </div>
...
const handleClickOutside = (event: MouseEvent) => {
  if (
    selectionBarRef.value &&
    !selectionBarRef.value.contains(event.target as Node)
  ) {
    isOpen.value = false;
  }
};
